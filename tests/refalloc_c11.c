...
int main(int argc, char **argv){
  ...
  /* make the threads */{
    ...
    for (thread_i = 0; thread_i < 4; ++thread_i){
      ...
      errnum = thrd_create
        (four_threads+thread_i, test_thread, thread_i_array+thread_i);
      if (errnum == thrd_success) continue;
      else {
  ...
  /* join threads */{
    ...
    for (thread_i = 0; thread_i < 4; ++thread_i){
      ...
      if (thread_i_array[thread_i] == (thread_i+1)){
        int result;
        errnum = thrd_join(four_threads[thread_i], &result);
        if (errnum == thrd_success) continue;
        else {
